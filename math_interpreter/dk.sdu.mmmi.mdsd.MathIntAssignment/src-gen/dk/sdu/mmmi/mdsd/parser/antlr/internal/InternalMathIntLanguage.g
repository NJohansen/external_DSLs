...
@after {
	...
	(
		...
		{
			$current = $this_MultiplyOrDivision_0.current;
			afterParserOrEnumRuleCall();
		}
		(
			(
...
entryRuleMultiplyOrDivision returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getMultiplyOrDivisionRule()); }
	iv_ruleMultiplyOrDivision=ruleMultiplyOrDivision
	{ $current=$iv_ruleMultiplyOrDivision.current; }
	EOF;

// Rule MultiplyOrDivision
ruleMultiplyOrDivision returns [EObject current=null]
@init {
	enterRule();
}
@after {
